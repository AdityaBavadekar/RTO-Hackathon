...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (id && username) {
      ...
      setUser({ id, ...rtoCenter, email: username, area: rtoCenter.rto_address, role: 'admin' });
    }
  },[]);
  ...
  const signUp = async (email: string, password: string, area: string, role: 'admin' | 'officer') => {
    setLoading(true);
    try {
      fetch('http://localhost:8000/api/rto/auth/register', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify(
          {
            ...
            "rto_main_email": email.endsWith('@example.com') ? email : 'rto_test@gmail.com',
  ...
  const signIn = async (email: string, password: string) => {
    ...
    try {
      ...
      fetch('http://localhost:8000/api/rto/auth/login', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
